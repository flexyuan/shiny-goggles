...
export const trackOptions = createTrackOptions(tracks);

function createTrackOptions(tracks) {
    const result = {};
    for (const track of tracks) {
        result[track.code] = track;
        result[track.name] = track;

        if (track.alt) {
            result[track.alt.code] = track;
            result[track.alt.name] = track;
        }
    }
    return result;
}
...
const options = {
    includeScore: true,  // Include the calculation score
    keys: ['code', 'name', 'alt.name', 'alt.code', {name: 'commonName'}],  // Specify which properties to search in
    threshold: 0.5,  // Adjust this for more or less fuzziness
};

export const fuse = new Fuse(tracks, options);